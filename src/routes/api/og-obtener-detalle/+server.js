...
/**
 * Obtiene el detalle de una solicitud desde la perspectiva del sujeto obligado en la PNT.
 *
 * Recibe un `idSolicitudDependencia` (entero) y un `token` Bearer, y los reenvía al
 * endpoint `consultaSolicitudDetalle` de la PNT junto con `idOrganoGarante: 31`,
 * devolviendo la respuesta tal cual.
 * Guarda cada respuesta exitosa en Upstash Redis como registro de acceso
 * con la clave `og-obtener-detalle`.
 *
 ...
 *
 * // Success response: JSON del detalle de la solicitud (vista sujeto obligado)
 * // Error responses:
...
export async function POST({ request }) {
	try {
		...
		const response = await fetch(
			'https://www.plataformadetransparencia.org.mx/inai3/rest/sujetosObligados/misSolicitudes/consultaSolicitudDetalle',
			{
				method: 'POST',
				headers: headers,
		...
		await redis.lpush('og-obtener-detalle', JSON.stringify({
		...
		console.error('Error in og-obtener-detalle API:', error);
		return json(
			{
